...
\vspace{-9pt}
\begin{myboxtitle}[Cammini minimi da sorgente unica]
\BI
...
\node[mynode, label={below:{$d'[h] \geq d[h]$}}] at (3.0,0.0) (b) {$h$};
...
\onslide<3|handout:3>
\BI
\item \makebox[2.5cm][l]{Per costruzione:} $d'[h] \geq d[h]$
...
\While{\NOT\ $Q.\setempty()$}
{
  \lnlset{line:shortest-remove}{(2)}$\INTEGER\ u = Q.\textsf{extract}()$\;
  $b[u] = \FALSE$\;
  \ForEach{$v \in G.\adj(u)$}
  {
    \If{$d[u] +G.w(u,v) < d[v]$}
