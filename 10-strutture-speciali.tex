...
\BB{Memorizzazione ($A[0 \ldots n-1]$)}
  \medskip
  \begingroup
  \renewcommand*{\arraystretch}{1.2}
  \begin{tabular}{ll}
...
\BB{Proprietà max-heap su vettore}
\[
A[i] \geq A[l(i)], A[i] \geq A[r(i)]
\]

}{
\BB{Proprietà min-heap su vettore}
\[
...
\BB{HeapSort è composto da tre funzioni}
\BIL
...
\pause
\medskip
\BB{Euristiche applicate agli insiemi disgiunti}
\BIL
...
\BB{Strategia per diminuire il costo dell'operazione \textsf{merge}()}
\BIL
...
\BB{Strategia per diminuire il costo dell'operazione \textsf{find}()}
\BIL
...
\begin{columns}[T]
\column{0.50\textwidth}
\BB{Durante le operazioni $\textsf{find}(x)$}
...
\BB{Quando si utilizzano entrambe le euristiche:}
\BIL
