...
\pause
\begin{mybox}
Algoritmo efficiente:\newline \alert{2$\lceil \log n \rceil$} 
\end{mybox}
}
...
\BB{
\BIL
\item 1817: Metodo della bisezione per trovare le radici di una funzione (Bolzano)
\item 1946: Prima menzione di binary search (John Mauchly, progettista di ENIAC)
\item 1960: Prima versione di binary search che lavora con vettori di dimensione arbitraria (!) (Derrick Henry Lehmer)
\EIL
}
...
\begin{frame}[shrink=10]{Problemi di overflow}
...
{
  \INTEGER $m = \alert{\lfloor i + (j-i)/2 \rfloor}$\;
  \uIf{$S[m] \Eq v$}{ 
    \Return $m$\; 
  }
  \uElseIf{$S[m] < v$}{
