...
$DP = \NEW\ \INTEGER[0 \mydots R]$\;
$S = \NEW\ \INTEGER[0 \mydots R]$\;
$DP[0] = 0$\;
\For{$i = 1$ \TO\ $R$} {
  $DP[i] = +\infty$\;
  \For{$j = 1$ \TO\ $n$} {
...
\vspace{-9pt}
\begin{myboxtitle}[Dimostrazione - Scelta greedy]
\BIL
\item Si consideri una permutazione ottima:

\[
\alert{B = [~B[1],B[2], \mydots, B[m-1], B[m], B[m+1], \mydots B[n]~]}
\]
...
\[
\alert{A = [~B[m], B[2], \mydots, B[m-1], B[1], B[m+1], \mydots, B[n]~]}
\]
...
di completamento medio di $B$
  \BI
	\item Job in posizione $1, \mydots, k-1$ in $A$ hanno tempo di completamento 
	$\leq$ dei job in posizione $1, \mydots, k-1$ in $B$
	\item Job in posizione $k, \mydots, n$ in $A$ hanno tempo di completamento 
	$=$ dei job in posizione $k, \mydots, n$ in $B$
  \EI
...
\begin{frame}{Algoritmo}
\begin{Procedure}
\caption[A]{$\REAL[\,]$\ \zaino($\REAL[\,]\ p,\ \REAL[\,]\ v,\REAL\ C,\ \INTEGER\ n$)}
