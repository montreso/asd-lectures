...
\BB{Algoritmi greedy (ingordi, golosi)}
\BI
...
\begin{frame}{Insieme indipendente di intervalli}
\TwoColsCustom{0.7}{0.25}{
\vspace{-18pt}
\begin{block}{Input}
Sia $S=\{ 1, 2, \ldots, n \}$ un insieme di intervalli della retta reale.
...
Trovare un \alert{insieme indipendente massimale} è un sottoinsieme di massima
...
\bigskip
\BB{\centering Confronta con "Insieme indipendente di intervalli pesati"}
\end{frame}
...
\BB{Iniziamo con programmazione dinamica}
\BI
...
\BB{Passiamo poi alla tecnica greedy}
\BI
...
\BB{Definizione ricorsiva della soluzione}
\[
	A[i,j] = A[i,k] \cup \{ k \} \cup A[k, j]
\]

\bigskip
\BB{Definizione ricorsiva del suo costo}
\BIL
...
\begin{overprint}
\onslide<1|handout:1>
\BB{Dimostrazione \circled{1}}

\smallskip
\makebox[3cm][l]{Sappiamo che:} \alert{$a_m<b_m$} \hfill (Definizione di intervallo)

\smallskip
\makebox[3cm][l]{Sappiamo che:} \alert{$\forall k \in S[i,j]: b_m \leq b_k$} \hfill ($m$ ha minor tempo di fine)

\smallskip
\makebox[3cm][l]{Ne consegue che:} \alert{$\forall k \in S[i,j]: a_m < b_k$} \hfill (Transitività)

\bigskip
Se nessun intervallo in $S[i,j]$ termina prima di $a_m$, allora $S[i,m] = \emptyset$
\onslide<2|handout:2>
\BB{Dimostrazione \circled{2}}
\BI
...
\BIL
\item Non è più necessario analizzare tutti i possibili valori di $k$
\BI
\item Faccio una scelta "ingorda", ma sicura: seleziono l'attività $m$ con il minor tempo di fine
\EI
\item Non è più necessario analizzare due sottoproblemi:
\BI
...
\BB{Dimostrazione - Scelta greedy}
\BIL
...
\begin{frame}{Algoritmo}
\begin{Procedure}
\caption[A]{\zaino($\REAL[\,]\ p,\ \REAL[\,]\ v,\REAL\ C,\ \INTEGER\ n,\ \REAL[\,]\ x$)}
\{ ordina $p$ e $v$ in modo che $p[1]/w[1] \ge p[2]/w[2] \ge \cdots \ge p[n]/w[n]$\}\;
\For{$i=1$ \TO\ $n$}
{
  $x[i] = \MIN(C/w[i], 1)$\;
  $C = C - x[i] \cdot w[i]$\;
...
\begin{block}{Alberi binari di decodifica}
\BI
...
\medskip
\BB{Quanti bit sono richiesti per codificare il file?}
\BI
...
\BB{Principio del codice di Huffman}
\BIL
...
\medskip
\BB{Un codice è progettato per un file specifico}
\BIL
...
c & \B{1}\R{0}\B{1} \\\hline
d & \B{1}\B{1}\B{1} \\\hline
e & \B{1}\B{1}\R{0}\R{0} \\\hline
f & \B{1}\B{1}\R{0}\B{1} \\\hline
\end{tabular}
}
\end{center}
}
...
\BB{Input}
\BI
\item $n$: dimensione dell'alfabeto
\item $c[1 \ldots n]$: caratteri alfabeto
\item $f[1 \ldots n]$: frequenze
\EI
...
\bigskip
\BB{Schema della dimostrazione}
\BIL
\item \alert{Proprietà della scelta greedy}
\BI
\item Scegliere i due elementi con la frequenza più bassa conduce sempre ad una soluzione ottimale
\EI
\item \alert{Sottostruttura ottima}
\BI
\item  Dato un problema sull'alfabeto $\Sigma$, è possibile costruire un sottoproblema con un alfabeto più piccolo
\EI
\EIL
\end{frame}
...
\begin{block}{Albero di copertura (Spanning tree)}
...
\begin{frame}{Definizione del problema}
\vspace{-6pt}
\begin{block}{Output: albero di copertura di peso minimo}
...
\BIL
\item $u,v$ sono connessi da un cammino $C \subseteq T$\\ (per definizione di albero)
...
$\INTEGER\ c = 0$\;
$\INTEGER\ i = 1$\;
\While(\REMF{Termina quando l'albero è costruito}){$c < n-1$ \AND $i \leq m$}{
  \If{$M.\mffind(A[i].u) \neq M.\mffind(A[i].v)$}
  {
...
\BB{Una struttura dati per i nodi non ancora nell'albero}
\BI
...
\bigskip
\BB{Albero registrato come \alert{vettore dei padri}}
\BI
...
\BIL
\item Inizializzazione: $O(n \log n)$
\item Il ciclo principale viene eseguito $\alert{O(n)}$ volte 
  \BI
	\item Ogni operazione $\textsf{extractMin}()$ costa $\alert{O(\log n)}$
	\EI
\item Il ciclo interno viene eseguito $\alert{O(m)}$ volte
  \BI
	\item Ogni operazione $\textsf{decreaseKey}()$ costa $\alert{O(\log n)}$
	\EI
\item  Tempo totale: \alert{$O(n+n \log n + m \log n)=O(m \log n)$}
	\BI
	\item Asintoticamente uguale a quello di Kruskal
	\EI
\EIL
...
\bigskip
\BB{Albero di copertura minima in un piano}
\BI
...
\BB{Applicazioni dirette nella progettazione di}
\BI
...
\bigskip
\BB{Alcuni utilizzi particolari}
\BI
