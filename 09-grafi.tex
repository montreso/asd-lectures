...
\While{\NOT $Q.\queueempty()$}
{
  ...
  \ForEach{$v \in G.\adj(u)$}
  {
    \If(\REMF{Se il nodo $v$ non è stato scoperto}){$\erdosn[v] == \infty$}
    {
      $\erdosn[v] = \erdosn[u]+1$\;
      $\mathit{parent}[v] = u$\;
      $Q.\queueinsert(v)$\;
...
\ForEach{$v \in G.\adj(u)$}
{
  \If{$\components[v] == 0$}{$\ccdfs(G, \mathit{counter}, v, \components)$}
}
\end{Procedure}
\end{columns}
\end{frame}
...
\ForEach{$v \in G.\adj(u)$}
{
  ...
  \uElseIf{$\ddt[u]>\ddt[v]$ \AND $\dft[v] == 0$}{
...
\ForEach{$v \in G.\adj(u)$}
{
  ...
  \ElseIf{$\ddt[u]>\ddt[v]$ \AND $\dft[v] == 0$}
  {
    \Return \TRUE\;
  }
}
...
\bigskip
\begin{tabular}{rl}
Arco dell'albero & \color{red}{$\ddt[v] == 0$}\\
