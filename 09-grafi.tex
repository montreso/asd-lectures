...
\begin{myboxtitle}[Problemi in grafi non pesati]
\BIL
...
\end{myboxtitle}
\medskip
\begin{myboxtitle}[Problemi in grafi pesati]
\BIL
...
\BI
\item $V$ è un insieme di \alert{nodi} (\alert{node}) o \alert{vertici} (\alert{vertex})
\item $E$ è un insieme di coppie non ordinate di nodi $(u,v)$ dette \alert{archi} o \alert{lati} (\alert{edge})
\EI
\end{myboxtitle}
...
\BlankLine
\INTEGER $\fontproc{size}()$\REMR{Restituisce il numero di nodi}
...
\TwoCols{
\begin{Procedure}
\caption[A]{\fontproc{BFSTraversal}($\Graph\ G, \INTEGER\ r$)}
...
\begin{Procedure}
\caption[A]{\fontproc{graphTraversal}($\Graph\ G, \Node\ r$)}
...
\begin{Procedure}
\caption[A]{\fontproc{printPath}($\Node\ r,\ \Node\ s,\ \Node[\,]\ \mathit{parent}$)}
...
\Else
{
  $\fontproc{printPath}(r, \mathit{parent}[s], \mathit{parent})$\;
  \textbf{print} $s$\;
}
\end{Procedure}
}
...
\item Quando un nodo viene scoperto:
  \BI
  \item  viene inserito nello stack con il tag \fontproc{discovery}
  \EI
\item Quando un nodo viene estratto dallo stack con tag \fontproc{discovery}:
  \BI
  \item Viene re-inserito con il tag \fontproc{finish}
  \item Tutti i suoi vicini vengono inseriti
  \EI
\item Quando un nodo viene estratto dallo stack con tag \fontproc{finish}:
  \BI
...
\ForEach{$v \in G.\adj(u) - \{ p \}$}
{
  ...
  \ElseIf{$\fontproc{hasCycleRec}(G, v, u, \mmark)$}
  {
    \Return \TRUE\;
  }
}
...
\ForEach{$u \in G.\VV()$}{
  \If{$\NOT\ \mmark[u]$}{
    \If{$\fontproc{hasCycleRec}(G, u, \NULL, \mmark)$}{
      \Return \TRUE\;
    }
  }
}
...
\begin{Procedure}
\caption[A]{$\BOOLEAN\ \fontproc{hasCycleRec}(\Graph\ G,\ \Node\ u, \INTEGER\ \&\orologio, \INTEGER[\,]\ \ddt, \INTEGER[\,]\ \dft)$}
...
\ForEach{$v \in G.\adj(u)$}
{
  \uIf{$\ddt[v] \Eq 0$}
  {
    \If{$\fontproc{hasCycleRec}(G, v, \orologio, \ddt, \dft)$}{\Return \TRUE} 
  }
...
\ForEach{$v \in G.\adj(u)$}
{
  \uIf{$\ddt[v] \Eq 0$}
  {
    \If{$\fontproc{hasCycleRec}(G, v, \orologio, \ddt, \dft)$}{\alert{\Return \TRUE}} 
  }
  \ElseIf{$\ddt[u]>\ddt[v]$ \AND $\dft[v] \Eq 0$}
  {
    \alert{\Return \TRUE}\;
...
\caption[A]{$\INTEGER[\,]$ \scc($\Graph\ G$)}
  ...
  $G^T = \fontproc{transpose}(G)$\REMR{Graph transposal}
...
\column{0.45\textwidth}
\begin{Procedure}
\caption[A]{\Graph \fontproc{transpose}($\Graph\ G$)}
...
\INTEGER\ $\mathit{counter} = 0$\;
\While{\alert{\NOT\ $S.\fontproc{isEmpty}()$}}
{
