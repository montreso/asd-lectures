...
%
\draw[edgen] (a) edge[above left,color=gray,dashed] node {\R{2}/\B{2}} (b);
\draw[edgen] (b) edge[right] node {\B{2}} (c);
\draw[edger] (a) edge[below left] node {\R{2}/\B{8}} (c);
\draw[edgen] (b) edge[above] node {\B{7}} (d);
\draw[edgen] (b) edge[above right] node {\R{2}/\B{3}} (e);

\draw[edger] (c) edge[below] node {\R{2}/\B{6}} (e);
\draw[edgen] (d) edge[above] node {\B{3}} (f);
\draw[edgen] (d) edge[above right] node {\B{2}} (e);
\draw[edger] (e) edge[below right,color=gray,dashed] node[rounded corners=2pt,fill=red!10]  {\R{4}/\B{4}} (f);
\end{tikzpicture}
