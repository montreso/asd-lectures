...
\bigskip
\BB{\alert{Domanda}: come trasformare questa sequenza di bit o questo numero in un valore compreso in $[0, m-1]$?}
...
\PROCEDURE{\Hash \hashconstructor(\INTEGER $\hashsize$)}
{
  ...
  \For{$i = 0$\ \TO\ $\hashsize-1$}{ $t.\mathit{keys}[i] = \hempty$}
  \Return $t$\;
}
\BlankLine
...
\INTEGER\ \PROCEDURE{\hashscan(\Item $k$, \BOOLEAN \hinsert)}
{
  ...
  \While{$\mathit{keys}[j] \neq k$ \AND\ $\mathit{keys}[j] \neq \hempty$ \AND\ $i<m$}
  {
    \If{$\mathit{keys}[j] \Eq \hcancel$ \AND\ $\mathit{delpos}\Eq m$}{$\mathit{delpos} = j$}
    $j = (j + H'(k)) \bmod m$\;
    $i = i+1$\; 
  }
  \eIf{$\hinsert$ \AND\ $\mathit{keys}[j] \neq k$ \AND $\mathit{delpos} < m$}{
...
\Item\ \PROCEDURE{\dictlookup(\Item $k$)}
{
  ...
  \eIf{$\mathit{keys}[i] \Eq k$}{\Return\ $\mathit{values}[i]$\;}{\Return\ $\hempty$\;}
}
\BlankLine
...
\PROCEDURE{\dictinsert(\Item $k$, \Item $v$)}
{
  ...
  \eIf{$\mathit{keys}[i] \Eq \hempty$ \OR\ $\mathit{keys}[i] \Eq \hcancel$ \OR\ $\mathit{keys}[i] \Eq k$}
  {
    $\mathit{keys}[i] = k$\;
    $\mathit{values}[i] = v$\;
  }
