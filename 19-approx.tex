...
\For{$i=0$ \TO\ $n$}{
  $\DP[i][0] = \TRUE$\REMR{Obiettivo raggiunto}
}
\For{$r=1$ \TO\ $k$}{
  $\DP[0][r] = \FALSE$\REMR{Valori terminati}
}
\For{$i=1$ \TO\ $n$}{
  \For{$r=1$ \TO $A[i]-1$}{
    $\DP[i][r] = \DP[i-1][r]$\REMR{Valore troppo grande}
  }
  \For{$r=A[i]$ \TO $k$}{
    $\DP[i][r] = \DP[i-1][r]\ \OR\ \DP[i-1][r-A[i]]$\REMR{Valore ok}
  }
}
\Return $\DP[n][k]$\;
\end{Procedure}
...
\uIf{$r \Eq 0$}{
  \Return $\TRUE$\REMR{Obiettivo raggiunto}
}
\uElseIf{$i \Eq 0$}{
  \Return $\FALSE$\REMR{Valori terminati}
}
\uElseIf{$A[i]>r$}{
  \Return $\fontproc{ssRec}(A,i-1,r)$\REMR{Valore troppo grande}
}
\Else{
  \Return $\fontproc{ssRec}(A,i-1,r)\ \OR\ \fontproc{ssRec}(A,i-1,r-A[i])$\REMR{Valore ok}
}
\end{Procedure}
...
%-------------------------------------------------------------------------
\begin{frame}{Subset Sum -- Memoization -- $O(nk)$, $O(2^n)$}
...
\alert{
\Else{
 \BOOLEAN $\mathit{res} = \DP.\dictlookup(\langle i,r \rangle)$\;
  \If{$\mathit{res} \Eq \Nil$}{
    $\mathit{res} = \fontproc{ssRec}(A,i-1,r,\DP)$\REMR{Valore non preso}
    \If{$A[i]<r$}{
      $\mathit{res} = \mathit{res}$ \OR\ $\fontproc{ssRec}(A,i-1,r-A[i],\DP)$\REMR{Valore preso}
    }
...
\onslide<2|handout:1>
\EE possibile ridurlo a \textsc{subset-sum} scegliendo come
...
\]
\onslide<3|handout:2>
Poichè è possibile risolverlo tramite un algoritmo pseudo-polinomiale, \textsc{partition} è debolmente \NP-completo
\end{overprint}
\end{myboxtitle}
...
\vspace{-9pt}
\begin{myboxtitle}[Algoritmo \textsc{first-fit} (Greedy)]
...
sostituendoli con due archi non compresi nel circuito\}}
\]
\end{myboxtitle}
...
\vspace{-9pt}
\begin{myboxtitle}[Situazione al passo $i$-esimo]
\BIL
...
\begin{frame}{Lower bound -- Versione 1}

\vspace{-9pt}
\begin{myboxtitle}[Costo effettivo della soluzione trovata finora]
...
\vspace{-9pt}
\begin{myboxtitle}[Lower bound per "uscire" dall'ultimo nodo]
...
\begin{frame}{Lower bound -- Versione 3}

\vspace{-9pt}
\begin{myboxtitle}[Lower bound per "uscire" dai nodi non ancora scelti]
...
\begin{frame}{Lower bound -- Versione 4}

\begin{overprint}

\onslide<1|handout:1>
\vspace{-9pt}
\begin{myboxtitle}[Lower bound per "attraversare" i nodi non ancora scelti]
...
\onslide<2|handout:2>
\vspace{-9pt}
\begin{myboxtitle}[Lower bound per tornare nel primo nodo]
...
\ForEach{$c \in \mathit{choices}$}{
  ...
  \eIf{$i<n$}{
    ...
    \If{$\mathit{cost} < \Mincost$}{
      $\Minsol = S$\REMR{Variabile globale}
      $\Mincost = \mathit{costo}$\REMR{Variabile globale}
    }
  }
  $R.\setinsert(c)$\;
}
