...
\bigskip
\BB{Esempio -- \textsc{clique} è fortemente \NP-completo}
\BIL
...
\bigskip
\BB{Esempio -- \textsc{subset-sum} è debolmente \NP-completo}
\BIL
...
\medskip
\BB{Domanda -- \textsc{partition} è debolmente \NP-completo?}
...
\bigskip
\BB{Domanda -- \textsc{3-partition} è debolmente \NP-completo?}
...
\vspace{-9pt}
\begin{block}{Commesso viaggiatore con disuguaglianze triangolari ($\Delta$-\textsc{tsp})}
...
\[
d_{ij} \le d_{ik} + d_{kj} \quad \textrm{per} \quad 1 \le i, j, k \le n
\]}
trovare un percorso
che, partendo da una qualsiasi città, attraversi ogni città esattamente una
volta e ritorni alla città di partenza, in modo che la distanza complessiva
...
\vspace{-9pt}
\begin{block}{Ricerca locale}
Sia $\pi$ un circuito Hamiltoniano del grafo completo derivante dal problema
\textsc{tsp}. Un'euristica ragionevole di ricerca locale può essere progettata
sulla base del seguente intorno:
\[
