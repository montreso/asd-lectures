...
\BB{Obiettivo: \alert{stimare la complessità in tempo degli algoritmi}}
\BIL
\item Definizioni
...
\BB{Obiettivo: \emph{riflettere sulla complessità dei problemi e degli algoritmi}}
\BI
...
\item \alert{Schönhage–Strassen} (1971)
  ...
  \EI
\item \alert{Martin Fürer} (2007)
  \BI
  \item Complessità $O(n \cdot \log n \cdot 2^{O(\log^* n)})$
  \EI
...
\BB{Obiettivo: \alert{valutare gli algoritmi in base alla tipologia dell'input}}
\BI
...
\bigskip
\begin{myboxtitle}[Costo computazionale: $O(n \log n)$]
\[
  T(n) = \begin{cases}
    c & n=1 \\
	2T(n/2) + dn  & n>1
